...
private final class InstalledCLITurnRunner {
    private static let safetyInstructions = """
        You are generating text inside Tonycast. Do not invoke tools, read files, inspect the \
    ...
    private func start(
        ...
        do {
            ...
            continuation.finish(
                throwing: AIProviderError.unavailable(
                    "Tonycast could not prepare its private AI workspace."))
            return
        }
        ...
        var grokPrompt: URL?
        if kind == .grok {
            let url = workspace.appending(path: "tonycast-prompt-\(UUID().uuidString).txt")
            do {
                ...
                continuation.finish(
                    throwing: AIProviderError.unavailable(
                        "Tonycast could not write its private AI prompt."))
                return
            }
            grokPrompt = url
    ...
    private func arguments(promptFile: URL? = nil) -> [String] {
        ...
        case .openCode:
            var result = [
                ...
                "--dir", workspace.path, "--title", "Tonycast"
            ]
