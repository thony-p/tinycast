    ...
    private var systemPromptSection: some View {
        ...
        return Section {
            Toggle(isOn: $settings.systemPromptEnabled) {
                ...
                Text("Off also skips Tonycast's own prompt.")
            }
            ...
            Text("Sent before every message, after Tonycast's own. Both are billed each turn.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
        ...
        if settings.enabledInstalledProviders.contains(kind) {
            ...
            case .notInstalled:
                LabeledContent {
                    ...
                    Text("Tonycast could not find the \(kind.command) command.")
                }
            case .failed(let message):
                LabeledContent {
                    providerActions {
