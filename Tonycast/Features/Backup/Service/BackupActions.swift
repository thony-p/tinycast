...
extension UTType {
    ...
    static let tonycastBackup = UTType(exportedAs: "com.tonycast.backup")
}
...
enum BackupActions {
    ...
    // MARK: - Tonycast native (own file panels; dialogs come from `AppCore`)
    ...
    static func chooseBackupFile() -> URL? { chooseFile(ofType: .tonycastBackup) }
    ...
    {
        guard let destination = chooseSaveLocation(named: "Tonycast", type: .tonycastBackup) else {
    ...
    private static let restartAfterImportText = "Quit and reopen Tonycast to finish."
