    ...
    enum InstallError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .noBuiltCommands(let name):
                return
                    "\(name) has no built command bundles. Tonycast installs prebuilt extensions — run `ray build` in the extension folder first, or import one from an installed Raycast."
