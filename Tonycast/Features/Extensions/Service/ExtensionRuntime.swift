...
final class ExtensionRuntime: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.tonycast.extensions.js", qos: .userInitiated)
    ...
    private func bootOnQueue(config: ExtensionBootConfig) throws {
        ...
        _ = context.objectForKeyedSubscript("__tonycast")?
    ...
    func start(
        ...
        await onQueue { context in
            let compiled = context.objectForKeyedSubscript("__tonycast")
    ...
    func popNavigation(session: String) async -> Bool {
        await withCheckedContinuation { continuation in
            queue.async {
                ...
                let result = context.objectForKeyedSubscript("__tonycast")?
    ...
    /// Installs `__tonycastHost` (the JS→Swift seam) and `__tonycastCompile`.
    private func installHost(in context: JSContext) {
        ...
        context.setObject(host, forKeyedSubscript: "__tonycastHost" as NSString)
        ...
        context.setObject(compile, forKeyedSubscript: "__tonycastCompile" as NSString)
    }
    ...
    private func startTimer(id: String, milliseconds: Double, repeats: Bool) {
        ...
        timer.setEventHandler { [weak self] in
            ...
            _ = self.context?.objectForKeyedSubscript("__tonycast")?
