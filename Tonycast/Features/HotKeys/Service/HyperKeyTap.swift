...
private enum CapsLockRemap {
    ...
    private static let queue = DispatchQueue(label: "com.tonycast.capslock-remap", qos: .utility)
    ...
    private static func apply(_ mapping: String) {
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                NSLog("Tonycast: hidutil remap exited %d", process.terminationStatus)
            }
        } catch {
            NSLog("Tonycast: hidutil caps lock remap failed: %@", error.localizedDescription)
        }
    }
}
