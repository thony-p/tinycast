...
extension AppEntry {
    ...
    init(_ layout: WindowLayout) {
        self.init(
            ...
            url: URL(string: "tonycast://window-layout/" + layout.id.uuidString)!,
    ...
    init(_ size: CustomWindowSize) {
        self.init(
            ...
            url: URL(string: "tonycast://window-size/" + size.id.uuidString)!,
    ...
    init(_ action: CustomQuickAction) {
        self.init(
            ...
            url: URL(string: "tonycast://quick-action/" + action.id.uuidString)!,
    ...
    init(_ command: CustomCommand) {
        self.init(
            ...
            url: URL(string: "tonycast://custom-command/" + command.id.uuidString)!,
    ...
    init(_ quicklink: Quicklink) {
        self.init(
            ...
            url: URL(string: "tonycast://quicklink/" + quicklink.id.uuidString)!,
...
final class AppIndex {
    ...
    private static let systemActionEntries: [AppEntry] = SystemActionCatalog.all
        .map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "tonycast://system-action/" + command.id.rawValue)!,
    ...
    private static let allWindowCommandEntries: [AppEntry] = WindowCommandCatalog.all
        .map { command in
            AppEntry(
                id: command.entryID, name: command.name,
                url: URL(string: "tonycast://window-command/" + command.id.rawValue)!,
                bundleID: nil, kind: .windowCommand)
        }
        .sorted { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
