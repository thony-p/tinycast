...
final class TextInjector {
    ...
    /// A hotkey's target comes from `frontmostApplication`, which can be Tonycast itself.
    ...
    func copySelection(
        ...
        clipboardManager.prepareForTonycastPasteboardMutation()
    ...
    private func restore(_ snapshot: PasteboardSnapshot, to pasteboard: any PasteboardAccess) {
        ...
        clipboardManager.synchronizeAfterTonycastPasteboardMutation(
    ...
    private func finish(_ lease: TemporaryPasteboardLease) {
        ...
        case .restored(let changeCount):
            ...
            clipboardManager.synchronizeAfterTonycastPasteboardMutation(changeCount: changeCount)
        case .superseded:
            break
        case .failed:
