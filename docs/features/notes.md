...
- **Tonycast is the only writer.** There is no watcher and no revision check: a save replaces the file
...
restores the prior external application or Tonycast window and flushes without delaying the order-out —
...
Command-Q is bound to nothing app-wide, so no chord over Notes can quit Tonycast.
...
state: editing the *active* note in another app while Tonycast has it open loses that edit the next time
