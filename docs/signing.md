...
Tonycast is signed with a **stable self-signed identity** called `Tonycast Self-Signed`. Keeping the
...
## 1. Create the `Tonycast Self-Signed` identity (once)
...
openssl pkcs12 -export -inkey /tmp/tc-key.pem -in /tmp/tc-cert.pem \
  -name "Tonycast Self-Signed" -out /tmp/tc.p12 -passout pass:tonycast
...
security import /tmp/tc.p12 -k ~/Library/Keychains/login.keychain-db \
  -P tonycast -A -T /usr/bin/codesign
...
```sh
security find-identity -p codesigning | grep "Tonycast Self-Signed"
```
...
```sh
gh secret set SIGNING_P12_BASE64   --repo abue-ammar/tonycast < /tmp/signing.p12.base64
gh secret set SIGNING_P12_PASSWORD --repo abue-ammar/tonycast --body "$P12_PASSWORD"
rm -f /tmp/signing.p12.base64   # holds your private key — delete it
```

If you ever lose the secrets, just re-run this section — as long as the `Tonycast Self-Signed`
...
`Tonycast Dev.debug.dylib` is refused at launch because a self-signed identity carries no Team ID for
the loader to match. The flag is not part of the designated requirement, so turning it on costs no
Accessibility grant. Each entitlement in `Tonycast/Tonycast.entitlements` earns its place:
...
| `com.apple.security.personal-information.calendars` | `requestFullAccessToEvents()` returns `false` in milliseconds with no dialog, and Tonycast never appears under System Settings › Calendars |
...
to its entitlement, including resources Tonycast does not use. That grants nothing — only
`Tonycast.entitlements` does, and a row whose usage string `Info.plist` doesn't declare is skipped. It
...
`BundleSignature` already accepts a bundle signed by the Tonycast team under Apple's Developer ID
chain, even though releases are still signed with `Tonycast Self-Signed`. That is deliberate and
...
`Tonycast Self-Signed`, so a contributor keeps building with the one they created in §1 — same name,
their own key, never shared. Nothing about local development changes.

**Keep `Tonycast Self-Signed` in the login keychain after the switch.** It is the only way to ship a
