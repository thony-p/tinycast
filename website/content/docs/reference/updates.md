---
title: Updates
description: How Tonycast finds, checks and installs new versions by itself.
---

Tonycast keeps itself up to date. There is nothing to set up.

## How it works

1. **Once a day**, Tonycast asks GitHub whether a newer release is out for its channel. If a check
...
4. Tonycast checks the download, installs it, and offers to **relaunch**.
...
launcher, the Tonycast menu bar menu, and **Settings → About**.
...
On an Intel Mac, Tonycast only installs the universal build. If a release has none, nothing is
...
- **The signature must prove the download is Tonycast**, signed the same way as the copy you are
  running.
- The app identifier and the version must be the ones expected.
- Tonycast never runs `xattr` or asks for an administrator password. If `/Applications` cannot be
...
Tonycast's Homebrew casks tell Homebrew that the app updates itself. So `brew upgrade` **skips
Tonycast on purpose**: it never reports it as outdated, never downloads it again, and never rolls a
...
Tonycast is free and open source. **Support Tonycast** in the launcher, the menu bar and
